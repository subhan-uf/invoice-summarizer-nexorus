...
export async function POST(request: NextRequest) {
  try {
    ...
    const { clientInfo, keyDetails, totalAmount, invoiceDate } = extractedData;
    ...
    if (clientInfo?.name || clientInfo?.email) {
      ...
      if (existingClient) {
        ...
        const newTotal =
          (existingClient.total_amount || 0) + (totalAmount || 0);
        const newCount = (existingClient.total_invoices || 0) + 1;

        const { data: _updatedClient } = await supabase
          .from("clients")
          .update({
    ...
    const { error: emailError } = await supabase.from("email_history").insert({
      user_id,
