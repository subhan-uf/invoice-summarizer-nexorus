...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "[delete-account API] Starting account deletion for user:",
      user.id,
    );
    ...
    for (const table of tablesToClean) {
      ...
      if (deleteError) {
        console.error(
          `[delete-account API] Error cleaning ${table}:`,
          deleteError,
        );
    ...
    const { error: authDeleteError } =
      await authenticatedSupabase.auth.admin.deleteUser(user.id);

    if (authDeleteError) {
      console.error(
        "[delete-account API] Error deleting user from auth:",
        authDeleteError,
      );
