...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: users, error: userError } =
      await authenticatedSupabase.auth.admin.listUsers();
    ...
    if (!user) {
      console.error(
        "[Store Gmail Tokens] User not found for email:",
        userEmail,
      );
    ...
    if (existingTokens) {
      ...
      if (updateError) {
        console.error(
          "[Store Gmail Tokens] Error updating tokens:",
          updateError,
        );
      ...
      console.log(
        "[Store Gmail Tokens] Updated existing tokens for user:",
        user.id,
      );
    } else {
      // Insert new tokens
      ...
      if (insertError) {
        console.error(
          "[Store Gmail Tokens] Error inserting tokens:",
          insertError,
        );
      ...
      console.log(
        "[Store Gmail Tokens] Inserted new tokens for user:",
        user.id,
      );
    }
