...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[summarize API] Supabase fetch invoice:", {
      invoice,
      fetchError,
    });

    if (fetchError || !invoice) {
      console.error("[summarize API] Invoice not found:", {
        invoiceId,
        fetchError,
      });
    ...
    if (invoice.summary && action === "summarize") {
      console.log(
        "[summarize API] Invoice already summarized:",
        invoice.summary,
      );
    ...
    console.log("[summarize API] Storage download:", {
      fileData,
      downloadError,
    });
    ...
    if (downloadError || !fileData) {
      console.error(
        "[summarize API] Failed to download PDF file:",
        downloadError,
      );
    ...
    console.log("[summarize API] Test user check:", {
      hasUser: !!user,
      userId: user?.id,
    });
    ...
    if (user) {
      ...
      console.log("[summarize API] Database test:", {
        testData,
    ...
    if (summary.clientInfo && summary.clientInfo.company && user) {
      console.log(
        "[summarize API] Looking up or creating client for company:",
        summary.clientInfo.company,
      );
    ...
    if (summary.clientInfo && summary.clientInfo.company) {
      ...
      console.log(
        "[summarize API] Setting client name for invoice:",
        summary.clientInfo.company,
      );
    ...
    if (summary.totalAmount) {
      ...
      const cleanAmount = summary.totalAmount
        .toString()
        .replace(/[$,€£¥\s]/g, "");
      ...
      if (!isNaN(numericAmount)) {
        ...
        console.log("[summarize API] Setting invoice amount:", {
          original: summary.totalAmount,
          numeric: numericAmount,
        });
      } else {
        console.warn(
          "[summarize API] Could not parse amount:",
          summary.totalAmount,
        );
      }
    }
    ...
    if (updateError) {
      console.error(
        "[summarize API] Failed to update invoice with summary:",
        updateError,
      );
    ...
    return NextResponse.json({
      ...
      message:
        action === "email"
          ? "Summary generated and sent via email"
          : updateMessage,
      summary: summary,
      updates: updates,
    });
